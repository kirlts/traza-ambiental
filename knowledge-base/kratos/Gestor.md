---
estado: borrador
tipo: norma_legal
vigencia: por_verificar
depende_de: "[[Ley 20.920 - Artículo 3]]"
se_descompone_en: []
se_relaciona_con: ["[[Oficio Ordinario 606]]", "[[SINADER]]", "[[Secretaría Regional Ministerial de Salud]]", "[[Firma Electrónica Avanzada]]", "[[Certificado de Valorización]]"]
cssclasses: [kb-node]
---

# Gestor

## Qué dice

Persona natural o jurídica (pública o privada) que realiza operaciones de manejo de residuos, y que debe estar explícitamente autorizada y registrada en conformidad a la normativa vigente para operar.

## Por qué existe

Este concepto se deriva de [[Ley 20.920 - Artículo 3]] porque es una definición legal del artículo que establece la taxonomía de actores del ecosistema REP. El Gestor es el actor operativo que ejecuta físicamente el manejo del residuo (recolección, transporte, valorización, disposición final). Para Trazambiental, los gestores son usuarios del sistema que registran las operaciones de manejo y cierran la trazabilidad inyectando datos en SINADER. Sin gestores autorizados, la cadena de custodia del NFU no puede cerrarse legalmente.

## Lógica de descomposición

Este concepto es atómico y no requiere mayor descomposición.

## Relaciones Horizontales

- [[SINADER]]: **Obligación Operativa:** Los gestores de residuos son los actores responsables de inyectar la data final de valorización o disposición en la plataforma SINADER, cerrando la trazabilidad.
- [[Secretaría Regional Ministerial de Salud]]: **Dependencia Operativa:** Un Gestor no puede operar físicamente instalaciones sin RCA y permisos otorgados por la SEREMI de Salud.
- [[Oficio Ordinario 606]]: **Gravamen Económico:** El Oficio 606 dictamina que la actividad del Gestor es un 'proceso industrial', obligándolo a pagar IVA.
- [[Firma Electrónica Avanzada]]: **Exención Operativa:** Para sus operarios en terreno y personal de patio, el Gestor no está obligado a proveer Firma Electrónica Avanzada individual. La operación diaria se valida mediante credenciales y ACL estándar delegadas, reservando la FEA exclusivamente para la firma corporativa de reportes consolidados por parte de su Representante Legal.
- [[Certificado de Valorización]]: **Emisor Exclusivo:** El Gestor es el único actor facultado jurídicamente para emitir este certificado una vez finalizado el proceso de reciclaje físico.

## Fuente original

Ley N° 20.920 (Artículo 3).

## Evidencia



## Justificación de estado

Permanece en `borrador` porque la evidencia primaria no ha sido enlazada y no se ha verificado formalmente el contenido contra el texto legal. Para avanzar a `verificado` se requiere enlazar la definición desde la BCN.
